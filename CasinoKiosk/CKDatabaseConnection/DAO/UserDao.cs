...
namespace CKDatabaseConnection.DAO
{
    public class UserDao
    {
        ...
        public void printGoldenHour(LocalReport report, int ID, string filename)
        {
            report = PrintExtention.ExportReportGoldenHour(report, ID);
            PrintExtention.Print(report);
          //  PrintExtention.SavePDF(report, filename);
        }

        public DataTable GetDataGoldenHourByID(int ID)
        {
            DataTable dt = new DataTable();
            if (pmCon.State == ConnectionState.Open) pmCon.Close();
            using (SqlCommand cmd = new SqlCommand("sp_HTRGoldenHourPromotionByID", pmCon))
            {
                pmCon.Open();
        ...
        public long TicketGoldenHourPrint(int PlayerID)
        {
            
            var ticket = new HTR_ThursdayGoldenHourPromotion();
            string userLogin = HttpContext.Current.Session["UserName"].ToString();
            HTR_ThursdayGoldenHourPlayers player = context.HTR_ThursdayGoldenHourPlayers.FirstOrDefault(item => item.PlayerID == PlayerID);
            if (player != null)
            {
                if (player.Quantity == 1)
                {
                    ...
                    context.HTR_ThursdayGoldenHourPromotion.Add(ticket);
                    context.SaveChanges();
                    return ticket.ID;
                }
            }
        ...
        public string TicketGoldenHourRePrint(int ID)
        {
            string userLogin = HttpContext.Current.Session["UserName"].ToString();
            HTR_ThursdayGoldenHourPromotion ticket = context.HTR_ThursdayGoldenHourPromotion.Find(ID);
        ...
        public long TicketGoldenHourVoid(int id)
        {
            var ticket = new HTR_ThursdayGoldenHourPromotion();

            ticket = context.HTR_ThursdayGoldenHourPromotion.FirstOrDefault(x => x.ID == id);
            ...
            if (ticket.isPrinted == true)
            {
                if (ticket.isVoided != true)
                {
                    ...
                    HTR_ThursdayGoldenHourPlayers playerInfo = context.HTR_ThursdayGoldenHourPlayers.FirstOrDefault(x => x.PlayerID == ticket.PlayerID);
                    playerInfo.Quantity = 1;
                    playerInfo.PrintedQty = 0;
                    context.SaveChanges();
                }
            }
            return ticket.ID;
        ...
        public long PrintSundayPromotion(int PlayerID)
        {

            var ticket = new HTR_SundayGoldenHourLog();
            string userLogin = HttpContext.Current.Session["UserName"].ToString();
            HTR_SundayGoldenHourPlayers player = context.HTR_SundayGoldenHourPlayers.FirstOrDefault(item => item.PlayerID == PlayerID);
            if (player != null)
            {
                if (player.Quantity == 1)
                {
                    ...
                    ticket.PrintedDate = DateTime.Now;
                    ticket.PrintedBy = userLogin;
                    context.HTR_SundayGoldenHourLog.Add(ticket);
                    context.SaveChanges();
                    return ticket.ID;
                }
            }
        ...
        public string ReprintSundayPromotion(int ID)
        {
            string userLogin = HttpContext.Current.Session["UserName"].ToString();
            HTR_SundayGoldenHourLog ticket = context.HTR_SundayGoldenHourLog.Find(ID);
        ...
        public long VoidSundayPromotion(int id)
        {
            var ticket = new HTR_SundayGoldenHourLog();

            ticket = context.HTR_SundayGoldenHourLog.FirstOrDefault(x => x.ID == id);
        ...
        public long MiniBuffetPrint(int PlayerID, string PlayerName)
        {

            MiniBuffet_GetNewClassicPlayerLogs log = new MiniBuffet_GetNewClassicPlayerLogs();
            string userLogin = HttpContext.Current.Session["UserName"].ToString();

            log.PlayerID = PlayerID;
            log.PlayerName = PlayerName;
            log.PrintedBy = userLogin;
            log.PrintedDate = DateTime.Now;
            log.isPrinted = true;
         
            context.MiniBuffet_GetNewClassicPlayerLogs.Add(log);
            context.SaveChanges();
            return log.ID;
        ...
        public void MiniBuffetRePrint(int ID)
        {
            string userLogin = HttpContext.Current.Session["UserName"].ToString();
            MiniBuffet_GetNewClassicPlayerLogs ticket = context.MiniBuffet_GetNewClassicPlayerLogs.Find(ID);
        ...
        public long MiniBuffetVoid(int id)
        {
            var ticket = new MiniBuffet_GetNewClassicPlayerLogs();

            ticket = context.MiniBuffet_GetNewClassicPlayerLogs.FirstOrDefault(x => x.ID == id);
